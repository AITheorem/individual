...
for checkpoint_filename in paths:
    ...
    # current_results = {}
    # cbb_patching(current_results, model, test_set, cbb_patch_configs)
    # cbb_results.setdefault(model.cfg.d_mlp, []).append(current_results)

    current_results = {}
    aba_patching(current_results, model, test_set, patch_configs)
    aba_results.setdefault(model.cfg.d_mlp, []).append(current_results)
...
for results, title in [(aba_results, "ABA Results")]:
    plot_violin(results, title, bw_adjust=0.35)
